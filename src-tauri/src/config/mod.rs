    ...
    pub fn load() -> Self {
        ...
        if let Ok(raw) = fs::read_to_string(config_file_path()) {
            if let Ok(stored) = serde_json::from_str::<Config>(&raw) {
                if !stored.vault_path.is_empty() {
                    cfg.vault_path = stored.vault_path;
                }
                if !stored.bind.is_empty() {
                    cfg.bind = stored.bind;
                }
                if !stored.base_path.is_empty() {
                    cfg.base_path = stored.base_path;
                }
                if !stored.auth_token.is_empty() {
                    cfg.auth_token = stored.auth_token;
                }
            }
        }

        // Env overrides
        if let Ok(v) = std::env::var("ZENVOY_VAULT_PATH") {
            cfg.vault_path = v;
        }
        if let Ok(v) = std::env::var("ZENVOY_BIND") {
            cfg.bind = v;
        }
        if let Ok(v) = std::env::var("ZENVOY_BASE_PATH") {
            cfg.base_path = v;
        }
        if let Ok(v) = std::env::var("ZENVOY_AUTH_TOKEN") {
            cfg.auth_token = v;
        }
