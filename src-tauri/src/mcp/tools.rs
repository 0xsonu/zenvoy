use crate::vault::{NoteFolder, Vault};
...
pub fn list_tools() -> Value {
    json!([
        ...
        tool(
            "write_note",
            "Overwrite a note's body",
            json!({"type":"object","properties":{"path":{"type":"string"},"body":{"type":"string"}},"required":["path","body"]})
        ),
        tool(
            "create_note",
            "Create a new note",
            json!({"type":"object","properties":{"folder":{"type":"string"},"title":{"type":"string"}},"required":[]})
        ),
        tool(
            "append_to_note",
        ...
        tool(
            "rename_note",
            "Rename a note",
            json!({"type":"object","properties":{"path":{"type":"string"},"title":{"type":"string"}},"required":["path","title"]})
        ),
        tool(
            "move_note",
            "Move a note to a folder",
            json!({"type":"object","properties":{"path":{"type":"string"},"folder":{"type":"string"}},"required":["path","folder"]})
        ),
        tool(
            "archive_note",
            "Archive a note",
        ...
        tool(
            "scan_all_tasks",
            "List all tasks across notes",
            json!({"type":"object","properties":{}})
        ),
        tool(
            "toggle_task",
            "Toggle a task's checked state",
            json!({"type":"object","properties":{"path":{"type":"string"},"line":{"type":"integer"}},"required":["path","line"]})
        ),
        tool(
            "list_folders",
            "List all folders",
        ...
        tool(
            "create_folder",
            "Create a subfolder",
            json!({"type":"object","properties":{"folder":{"type":"string"},"subpath":{"type":"string"}},"required":["folder","subpath"]})
        ),
        tool(
            "rename_folder",
            "Rename a folder",
            json!({"type":"object","properties":{"folder":{"type":"string"},"old":{"type":"string"},"new":{"type":"string"}},"required":["folder","old","new"]})
        ),
        tool(
            "delete_folder",
            "Delete a folder",
        ...
        tool(
            "list_assets",
            "List all assets/attachments",
            json!({"type":"object","properties":{}})
        ),
        tool(
            "insert_at_line",
            "Insert text at a specific line",
            json!({"type":"object","properties":{"path":{"type":"string"},"line":{"type":"integer"},"text":{"type":"string"}},"required":["path","line","text"]})
        ),
        tool(
            "replace_in_note",
...
pub fn call_tool(vault: &Vault, name: &str, args: Value) -> Result<String, String> {
    match name {
        ...
        "read_note" => {
            let content = vault
                .read_note(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&content).unwrap())
        }
        "write_note" => {
            let meta = vault
                .write_note(&s(&args, "path"), &s(&args, "body"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "create_note" => {
            let folder_str = s(&args, "folder");
            let folder = if folder_str.is_empty() {
                NoteFolder::Inbox
            } else {
                NoteFolder::parse(&folder_str).ok_or("invalid folder")?
            };
            let title = args.get("title").and_then(|v| v.as_str());
            let meta = vault
                .create_note(&folder, title, None)
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "append_to_note" => {
            let meta = vault
                .append_to_note(&s(&args, "path"), &s(&args, "body"), "end")
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "prepend_to_note" => {
            let meta = vault
                .append_to_note(&s(&args, "path"), &s(&args, "body"), "prepend")
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "rename_note" => {
            let meta = vault
                .rename_note(&s(&args, "path"), &s(&args, "title"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "move_note" => {
            let folder = NoteFolder::parse(&s(&args, "folder")).ok_or("invalid folder")?;
            let meta = vault
                .move_note(&s(&args, "path"), &folder, None)
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "archive_note" => {
            let meta = vault
                .archive_note(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "unarchive_note" => {
            let meta = vault
                .unarchive_note(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "move_to_trash" => {
            let meta = vault
                .move_to_trash(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "restore_from_trash" => {
            let meta = vault
                .restore_from_trash(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "empty_trash" => {
        ...
        "delete_note" => {
            vault
                .delete_note(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(json!({"status":"deleted"}).to_string())
        }
        "duplicate_note" => {
            let meta = vault
                .duplicate_note(&s(&args, "path"))
                .map_err(|e| e.to_string())?;
            Ok(serde_json::to_string(&meta).unwrap())
        }
        "search_text" => {
            let results = vault
                .search_vault_text(&s(&args, "query"), None)
                .map_err(|e| e.to_string())?;
        ...
        "create_folder" => {
            let folder = NoteFolder::parse(&s(&args, "folder")).ok_or("invalid folder")?;
            vault
                .create_folder(&folder, &s(&args, "subpath"))
                .map_err(|e| e.to_string())?;
            Ok(json!({"status":"ok"}).to_string())
        }
        "rename_folder" => {
            let folder = NoteFolder::parse(&s(&args, "folder")).ok_or("invalid folder")?;
            let result = vault
                .rename_folder(&folder, &s(&args, "old"), &s(&args, "new"))
                .map_err(|e| e.to_string())?;
        ...
        "delete_folder" => {
            let folder = NoteFolder::parse(&s(&args, "folder")).ok_or("invalid folder")?;
            vault
                .delete_folder(&folder, &s(&args, "subpath"))
                .map_err(|e| e.to_string())?;
            Ok(json!({"status":"deleted"}).to_string())
        }
        "list_assets" => {
