...
pub fn protected_routes(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            "/vault/settings",
            get(vault_settings).post(set_vault_settings),
        )
...
async fn set_vault_settings(
    State(state): State<Arc<AppState>>,
    Json(body): Json<VaultSettings>,
...
#[derive(Deserialize)]
struct SelectVaultReq {
    path: String,
}

async fn select_vault(
    State(state): State<Arc<AppState>>,
    Json(body): Json<SelectVaultReq>,
) -> Result<Json<Value>, StatusCode> {
    ...
    let new_watcher =
        VaultWatcher::start(new_vault.root()).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn browse_directories(
    ...
    let entries: Vec<BrowseEntry> = std::fs::read_dir(&target)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
        .filter_map(|e| e.ok())
...
async fn create_note(
    ...
    let vault = get_vault(&state)?;
    let folder = crate::vault::NoteFolder::parse(&req.folder).ok_or(StatusCode::BAD_REQUEST)?;
    vault
        .create_note(&folder, req.title.as_deref(), req.subpath.as_deref())
        .map(|m| Json(serde_json::to_value(m).unwrap()))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn rename_note(
    ...
    let vault = get_vault(&state)?;
    vault
        .rename_note(&req.path, &req.title)
        .map(|m| Json(serde_json::to_value(m).unwrap()))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn move_note(
    ...
    let folder = crate::vault::NoteFolder::parse(&req.folder).ok_or(StatusCode::BAD_REQUEST)?;
    vault
        .move_note(&req.path, &folder, req.subpath.as_deref())
        .map(|m| Json(serde_json::to_value(m).unwrap()))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn list_folders(State(state): State<Arc<AppState>>) -> Result<Json<Value>, StatusCode> {
    ...
    vault
        .list_folders()
        .map(|f| Json(serde_json::to_value(f).unwrap()))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn create_folder(
    ...
    let folder = crate::vault::NoteFolder::parse(&req.folder).ok_or(StatusCode::BAD_REQUEST)?;
    vault
        .create_folder(&folder, &req.subpath)
        .map(|_| Json(json!({"ok": true})))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn rename_folder(
    ...
    let folder = crate::vault::NoteFolder::parse(&req.folder).ok_or(StatusCode::BAD_REQUEST)?;
    vault
        .rename_folder(&folder, &req.old, &req.new)
        .map(|s| Json(json!({"subpath": s})))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}

#[derive(Deserialize)]
struct DeleteFolderReq {
    folder: String,
    subpath: String,
}
...
async fn delete_folder(
    State(state): State<Arc<AppState>>,
    Json(req): Json<DeleteFolderReq>,
) -> Result<Json<Value>, StatusCode> {
    let vault = get_vault(&state)?;
    let folder = crate::vault::NoteFolder::parse(&req.folder).ok_or(StatusCode::BAD_REQUEST)?;
    vault
        .delete_folder(&folder, &req.subpath)
        .map(|_| Json(json!({"ok": true})))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}

#[derive(Deserialize)]
struct DuplicateFolderReq {
    folder: String,
    subpath: String,
}
...
async fn duplicate_folder(
    State(state): State<Arc<AppState>>,
    Json(req): Json<DuplicateFolderReq>,
) -> Result<Json<Value>, StatusCode> {
    let vault = get_vault(&state)?;
    let folder = crate::vault::NoteFolder::parse(&req.folder).ok_or(StatusCode::BAD_REQUEST)?;
    vault
        .duplicate_folder(&folder, &req.subpath)
        .map(|s| Json(json!({"subpath": s})))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn read_comments(
    ...
    let vault = get_vault(&state)?;
    vault
        .read_note_comments(&q.path)
        .map(|c| Json(serde_json::to_value(c).unwrap()))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}

#[derive(Deserialize)]
struct WriteCommentsReq {
    path: String,
    comments: Vec<crate::vault::NoteComment>,
}

async fn write_comments(
    State(state): State<Arc<AppState>>,
    Json(req): Json<WriteCommentsReq>,
) -> Result<Json<Value>, StatusCode> {
    let vault = get_vault(&state)?;
    vault
        .write_note_comments(&req.path, req.comments)
        .map(|c| Json(serde_json::to_value(c).unwrap()))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}
...
async fn search_capabilities(
    ...
    let vault = get_vault(&state)?;
    Ok(Json(
        serde_json::to_value(vault.get_text_search_capabilities()).unwrap(),
    ))
}
...
async fn search_text(
    ...
    let vault = get_vault(&state)?;
    vault
        .search_vault_text(&q.query, q.backend.as_deref())
...
async fn watch_ws(ws: WebSocketUpgrade, State(state): State<Arc<AppState>>) -> impl IntoResponse {
