  ...
  return (
    <div className="flex h-[calc(100vh-2.75rem)] items-center justify-center">
      <div className="flex max-w-md flex-col items-center gap-5 text-center">
        ...
        />
        <div>
          <h1 className="font-serif text-2xl font-semibold text-ink-900">Welcome to Zenvoy</h1>
          <p className="mt-2 text-sm text-ink-600">
            {isServerVaultSetup
              ? 'Choose the vault directory on the server running Zenvoy. The normal self-hosted path is `make up`, which serves the browser app and server together.'
              : 'Choose a folder on your computer to use as your vault. Zenvoy will store your notes there as plain markdown files — yours to keep, back up, and sync any way you like.'}
          </p>
          {isServerVaultSetup && (
          ...
          {canConnectRemote && (
            <p className="mt-2 text-xs text-ink-500">
              You can open a local vault on this machine or connect the desktop app to a Zenvoy
              server.
            </p>
          )}
        </div>
