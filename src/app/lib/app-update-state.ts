...
export function appUpdateNoticeLabel(state: AppUpdateState | null): string | null {
  switch (state?.phase) {
    case 'available':
      return `Zenvoy ${state.availableVersion ?? 'update'} is available`
    case 'downloaded':
      return `Zenvoy ${state.availableVersion ?? 'update'} is ready`
    case 'downloading':
      return `Downloading Zenvoy ${state.availableVersion ?? 'update'}`
    default:
      return null
  }
}
