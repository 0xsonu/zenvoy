...
const storeState = vi.hoisted(() => ({
  ...
  assetFiles: [
    {
      path: 'zenvoy logo.png',
      name: 'zenvoy logo.png',
      kind: 'image' as const,
      siblingOrder: 0,
      size: 100,
      updatedAt: 1
    ...
    {
      path: 'media/zenvoy-demo-card.svg',
      name: 'zenvoy-demo-card.svg',
      kind: 'image' as const,
      siblingOrder: 1,
      size: 100,
      updatedAt: 1
...
describe('wikilinkSource', () => {
  it('offers asset files as wikilink completions', () => {
    ...
    expect(result?.options.map((option) => option.label)).toEqual(
      expect.arrayContaining(['Zen Garden', 'zenvoy logo.png'])
    )
  })
  ...
  it('inserts selected image assets as embeds', () => {
    ...
    const result = wikilinkSource(new CompletionContext(view.state, view.state.doc.length, true))
    const option = result?.options.find((candidate) => candidate.label === 'zenvoy logo.png')
    ...
    expect(view.state.doc.toString()).toBe('![[zenvoy logo.png]]')
    view.destroy()
    parent.remove()
  })
})
