...
export const HELP_QUICK_START: HelpCard[] = [
  {
    ...
    body:
      'A vault is just a folder of markdown files. Zenvoy reads and writes that folder directly, so your notes stay portable, syncable, and readable outside the app too.'
  },
  {
  ...
  {
    title: 'Use files without leaving Zenvoy',
    body:
      'Images, SVGs, PDFs, audio, video, and other local files can appear in the vault tree and open in Zenvoy tabs or reference panes. The files stay ordinary vault files, but opening them does not have to bounce you out to another app.'
  },
  {
    title: 'Pick up where you left off',
    body:
      'Zenvoy restores open tabs, splits, built-in views, and sidebar layout per vault. It also remembers the main window bounds, so reopening the app feels like returning to a workspace rather than starting over.'
  }
]
...
export const HELP_HOW_TO_GUIDES: HelpCard[] = [
  {
    ...
    body:
      'Use the Quick Note shortcut or the command palette entry to create a fast capture note. If date-titled Quick Notes are enabled, Zenvoy names it from today’s date automatically; otherwise it creates a normal quick note and focuses the title so you can keep moving.'
  },
  {
  ...
  },
  {
    ...
    body:
      'Open a note in a floating window when you want it in a separate OS window, or pin a note or PDF as a reference when you want it attached to the current writing context inside Zenvoy.'
  },
  {
    title: 'Search from Raycast on macOS',
    body:
      'Install the `zen` CLI from Settings → CLI, then use the Raycast Extension section on the same page to install Zenvoy for Raycast locally. Raycast can search notes, filter by folder or tag, open a note in the app, open it in a floating window, archive or unarchive, move to Trash, reveal in Finder, copy the note path, and copy a wikilink.'
  },
  {
    title: 'Check for updates and install them',
    body:
      'Use Check for Updates from the app menu, the command palette, or Settings → About. When a release is available, Zenvoy can download it in the background and then prompt you to install and relaunch.'
  },
  {
    title: 'Run the self-hosted web version with Docker',
    body:
      'Prefer Zenvoy in a browser instead of the desktop app? Pull the prebuilt, multi-arch image from Docker Hub with `docker pull adibhanna/zenvoy`, generate a login token and keep a copy (`openssl rand -hex 32`), then start the container with your vault mounted:\n`docker run -d -p 127.0.0.1:7878:7878 \\\n  -e ZENVOY_AUTH_TOKEN=<your-token> \\\n  -v "$HOME/Documents/MyVault:/workspace" \\\n  -v "$HOME/zenvoy-data:/data" \\\n  adibhanna/zenvoy:latest`\nThe server binds to 0.0.0.0, so it will not start without that token — open http://localhost:7878 and paste the token on first connect. Your notes stay as ordinary .md files on the host, and the desktop app can point at the same server. The full walkthrough, including reverse-proxy and TLS hardening, lives at /docs.'
  }
]
...
export const HELP_CORE_CONCEPTS: HelpCard[] = [
  {
    ...
    body:
      'Zenvoy edits markdown on disk. Rename, move, archive, restore, and floating-window operations all work on the underlying files, not an internal copy.'
  },
  {
  ...
  },
  {
    ...
    body:
      'Zenvoy treats context menus as keyboard-reachable UI, not mouse-only escape hatches. Use the configured context-menu binding on the selected sidebar or note-list row. In the editor, select text and press `m` to open the text menu with commenting first; `Shift+F10` / the system Context Menu key still opens the active tab menu when no text is selected. The command palette also exposes the same high-value tab actions directly.'
  },
  {
    title: 'Comments attach to selected text',
    body:
      'Select text in the editor and use the text menu to add a comment. Zenvoy stores note comments beside the note in vault metadata, then highlights the anchored text and line when the comment is active.'
  },
  {
  ...
  },
  {
    ...
    body:
      'When you type `/` at the start of a line or after whitespace, Zenvoy opens an inline insert menu for common markdown structures such as headings, bulleted or numbered lists, to-do items, callouts, code blocks, dividers, tables, math blocks, links, images, and even creating a new note page.'
  },
  {
  ...
  },
  {
    ...
    body:
      'Templates turn a repeated note shape into one keystroke. Zenvoy ships built-in templates for engineering (ADR, RFC, Bug Report, Postmortem, Meeting Notes, 1:1) and personal use (Daily Note, Weekly Review, Reading Notes, Journal, Project Kickoff, To-do), and you can author your own under Settings → Templates. A template is plain markdown with optional frontmatter and variables — `{{title}}`, `{{date}}`, `{{date:FORMAT}}`, `{{time}}`, `{{week}}`, and `{{cursor}}` — substituted at creation time. Custom templates are stored as `.md` files in `.zenvoy/templates/`, so they stay portable like everything else. Daily and weekly notes can each be assigned a template so dated notes start pre-filled.'
  },
  {
  ...
  },
  {
    ...
    body:
      'Drop files into a note to insert local files. By default, Zenvoy keeps them as ordinary files in the vault root, can reveal them from the app, and opens images, SVGs, PDFs, audio, video, and generic files inside Zenvoy tabs or reference panes where possible.'
  },
  {
    title: 'The CLI is the bridge to launchers',
    body:
      'The `zen` command-line tool can list, read, search, capture, edit, archive, trash, inspect tasks, and start the MCP server without the app running. Raycast uses it for search, then uses `zenvoy://open` and `zenvoy://open-window` links to bring the selected note back into Zenvoy. On macOS, Settings → CLI can install the bundled Raycast extension locally so users do not need to wait for the Raycast Store version.'
  },
  {
...
export const HELP_SETTINGS: HelpSettingsSection[] = [
  ...
  },
  {
    title: 'Vault',
    items: [
      { label: 'Vault location', detail: 'Reveal or change the root folder Zenvoy treats as the active vault.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Create a custom template', detail: 'Author a new template as markdown with optional frontmatter (`name`, `description`, `category`, `titleTemplate`, `targetFolder`, `targetSubpath`) and variables like `{{title}}`, `{{date}}`, `{{date:FORMAT}}`, `{{time}}`, `{{week}}`, and `{{cursor}}`. It is saved as a `.md` file in `.zenvoy/templates/`.' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Raycast on macOS', detail: 'The Raycast extension requires `zen` and can be installed locally from this settings page. Zenvoy copies the bundled extension into app data, installs dependencies, builds it, and imports it into Raycast. It searches with `zen list --json`, then opens notes in Zenvoy through `zenvoy://open` or `zenvoy://open-window` and exposes archive, unarchive, trash, reveal, copy path, and copy wikilink actions from Raycast.' },
      { label: 'Uninstall', detail: 'Removes only the Zenvoy-managed symlink — never an arbitrary unmanaged binary named `zen`. The CLI stays inside the app bundle for next time.' }
    ]
  },
  {
    title: 'About',
...
export const HELP_CLI: HelpCard[] = [
  ...
  },
  {
    ...
    body:
      'Open Settings → CLI and click Install. Zenvoy symlinks the bundled wrapper into a usable PATH location, preferring user-writable directories and only asking for admin access when no writable PATH target is available. After that, `zen --help` works in any new terminal. You can also run the install from the command palette via "Install Command-Line Tool (zen)".'
  },
  {
    title: 'No app required',
    body:
      'The CLI reads from the same vault folder the desktop app uses, so it works whether or not Zenvoy is open. When the app is open, file watchers pick up CLI changes automatically — captures and edits show up live in the sidebar.'
  },
  {
  ...
  },
  {
    ...
    body:
      'On macOS, install the Raycast extension locally from Settings → CLI after `zen` is installed. Zenvoy copies the bundled extension into app data, runs the local build, and imports it into Raycast, so you do not need the Raycast Store version. The Search Notes command reads from `zen list --json`, then uses `zenvoy://open` to open notes in the main app or `zenvoy://open-window` to open a floating window. Cmd+K actions also archive, unarchive, move to Trash, reveal in Finder, copy the path, and copy a wikilink.'
  },
  {
    title: 'Raycast local install requirements',
    body:
      'The local Raycast installer is macOS-only. It needs Raycast, the `zen` CLI, Node.js 22.14 or newer, and npm 7 or newer available from your login shell. Settings shows each requirement, the local extension path, and whether the installed copy is current with the bundled Zenvoy version.'
  },
  {
  ...
  },
  {
    ...
    body:
      '`zen mcp` starts the Zenvoy MCP server in stdio mode — the same one Claude Code, Claude Desktop, and Codex use under the hood. Once `zen` is installed, Settings → MCP installs configure the clients to launch `zen mcp` directly, so the install path is one stable absolute path that survives app moves.'
  }
]
