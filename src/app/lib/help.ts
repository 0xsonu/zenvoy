...
export const HELP_HOW_TO_GUIDES: HelpCard[] = [
  ...
  },
  {
    ...
    body:
      'Prefer Zenvoy in a browser instead of the desktop app? Pull the prebuilt, multi-arch image from Docker Hub with `docker pull 0xsonu/zenvoy`, generate a login token and keep a copy (`openssl rand -hex 32`), then start the container with your vault mounted:\n`docker run -d -p 127.0.0.1:7878:7878 \\\n  -e ZENVOY_AUTH_TOKEN=<your-token> \\\n  -v "$HOME/Documents/MyVault:/workspace" \\\n  -v "$HOME/zenvoy-data:/data" \\\n  0xsonu/zenvoy:latest`\nThe server binds to 0.0.0.0, so it will not start without that token — open http://localhost:7878 and paste the token on first connect. Your notes stay as ordinary .md files on the host, and the desktop app can point at the same server. The full walkthrough, including reverse-proxy and TLS hardening, lives at zenvoy.org/docs.'
  }
]
