...
interface Store {
  ...
  /** Custom templates loaded from `.zenvoy/templates/` (built-ins are constants). */
  customTemplates: NoteTemplate[]
  query: string
  initialized: boolean
...
async function ensureWebServerSession(
  ...
  const token = await promptApp({
    ...
    description:
      'This Zenvoy server requires its auth token before notes can be accessed in the browser.',
...
function describeWebServerSetupError(err: unknown): string {
  ...
  if (
    ...
    return [
      'Zenvoy could not reach its server API.',
...
export const useStore = create<Store>((set, get) => {
  ...
  saveActiveNoteAsTemplate: async () => {
    ...
    const name = await promptApp({
      ...
      description: 'Saved to .zenvoy/templates and shown in the template picker.',
        ...
        const enteredPath = await pickServerDirectoryApp(
          {
            ...
            description:
              'Choose the folder on the server that Zenvoy should use as your vault.',
  ...
  connectRemoteWorkspace: async () => {
    try {
      ...
      const baseUrl = await promptApp({
        title: 'Connect to Zenvoy Server',
        description:
          'Enter the base URL for the Zenvoy server, for example `http://localhost:7878` or `https://notes.example.com`.',
      ...
      const authToken = await promptApp({
        ...
        description:
          matchingBaseProfile?.hasCredential
            ...
            : 'If your Zenvoy server requires a bearer token, enter it here. Otherwise leave this blank.',
        ...
        const enteredPath = await pickServerDirectoryApp(
          {
            title: 'Choose Vault Folder',
            description:
              'Choose the folder on the connected Zenvoy server that should be used as your vault.',
            confirmLabel: 'Choose Folder'
          },
          async (selectedPath) => {
  ...
  changeRemoteWorkspaceVaultPath: async () => {
    try {
      ...
      if (!remoteInfo?.capabilities?.supportsVaultSelection) {
        throw new Error('This Zenvoy server does not allow switching vault folders from the app.')
      }
      ...
      const enteredPath = await pickServerDirectoryApp(
        {
          title: 'Choose Vault Folder',
          description:
            'Choose the folder on the connected Zenvoy server that Zenvoy should use as your vault.',
