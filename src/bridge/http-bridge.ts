...
const appPackage = { name: 'zenvoy-web', version: '1.0.0', description: 'Zenvoy web client', homepage: 'https://' }
import {
  installZenBridge,
  type ZenAppInfo,
  type ZenBridge,
...
// Base path under which the server is mounted (e.g. "/zenvoy" when
// running behind a reverse proxy at example.com/zenvoy/). The Go
// server injects a `<meta name="zn-base-path" content="...">` tag into
// the HTML shell when a non-empty `ZENVOY_BASE_PATH` is configured;
...
function wrapRouteUpgradeError(path: string, err: unknown): never {
  if (
    ...
    throw new Error(
      'Your Zenvoy server is running an older build and does not support the new vault picker yet. Restart `npm run dev:server` and reload the page.'
    )
  }
  ...
  if (!res.ok) {
    if (res.status === 401) {
      throw new HttpRequestError(
        res.status,
        path,
        'This Zenvoy server requires you to sign in with its auth token.'
      )
    }
...
async function pickVault(): Promise<VaultInfo | null> {
  ...
  const nextPath = window.prompt(
    'Enter the path to the vault directory on the server running Zenvoy.',
    suggested
  )
...
async function exportNotePdf(_relPath: string): Promise<string | null> {
  ...
  const exportWindow = window.open(url.toString(), 'zenvoy-pdf-export')
  if (!exportWindow) {
    throw new Error(
      'Zenvoy could not open the PDF export window. Allow pop-ups for this site and try again.'
    )
  }
  exportWindow.focus()
  return null
...
function notifyRendererReady(): void {
  // Desktop uses this to flush queued zenvoy:// deep-link requests.
  // Browser builds do not register the zenvoy:// protocol.
}
