/**
 * Shared descriptors for the MCP client integrations Zenvoy knows
...
export interface McpClientDescriptor {
   ...
   */
  format: 'json' | 'toml'
  /** Key Zenvoy uses inside the client's config — stable forever
   *  so re-runs upgrade the existing entry instead of creating a
   *  duplicate. */
  serverKey: string
}

export const MCP_SERVER_KEY = 'zenvoy'
...
export const MCP_CLIENTS: McpClientDescriptor[] = [
  ...
  },
  {
    ...
    label: 'Codex CLI',
    description:
      'OpenAI\u2019s Codex CLI. Appends a [mcp_servers.zenvoy] entry to ~/.codex/config.toml.',
    format: 'toml',
    serverKey: MCP_SERVER_KEY
  }
]
...
export interface McpClientStatus {
  ...
  configPath: string
  /** True if the config file currently contains a Zenvoy entry. */
  installed: boolean
